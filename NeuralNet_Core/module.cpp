...
void Module::register_module(std::string name, std::shared_ptr<Module> param)
{
...
void Module::load_state_dict(std::unordered_map<std::string, std::shared_ptr<Tensor>> &state_dict)
{
    for (const auto& p : _parameters)
    {
        ...
        {
            std::cerr << "Warning: Parameter '" + p.first + "' not found in state_dict" << std::endl;
            continue;
        }
